...
export async function generateStoryCards({ location = {}, place = null, reviews = null, context = {} } = {}) {
  ...
  const selectedPlace = place || context.place || null;
...
export async function getPlaceStory(place = {}, context = {}) {
  ...
  const factualParts = [
    ...
    place.description,
    place.summary,
    place.address ? `${normalizePlaceName(place)}은 ${place.address}에 있습니다.` : "",
    place.categoryName || place.type ? `분류상 ${place.categoryName || place.type}에 가까운 장소입니다.` : "",
  ].filter(Boolean);

  if (!factualParts.length) return null;

  const summary = factualParts[0];
  const script = [
    `${normalizePlaceName(place)}에 대해 현재 확인된 정보만 짧게 정리합니다.`,
    ...factualParts.slice(0, 3),
    "이 카드는 확인된 장소 정보에 기반하므로, 지역 설화나 역사 카드와는 별도로 들을 수 있습니다.",
  ].join(" ");
...
function makeReviewBasedPlaceStorySeed(place = {}, context = {}) {
  ...
  const snippets = reviews.slice(0, 2).map((review) => review.text).join(" ");
  return `${name}에 대한 로컬 리뷰를 보면 ${snippets}`;
}
...
export async function getLocalReviewStory(place = {}, reviews = null, context = {}) {
  ...
  return makeCard({
    ...
    type: "local_review",
    title: `방문자들이 말하는 ${normalizePlaceName(place)}의 매력`,
...
function getVisitTipStory(place = {}, location = {}) {
  ...
  if (place.distance) tips.push(`현재 위치에서 약 ${Math.max(1, Math.round(Number(place.distance)))}미터 거리로 표시됩니다.`);
  ...
  if (/FD6|음식|식당|맛집|카페|CE7/i.test(`${place.category || ""} ${place.categoryName || ""} ${place.type || ""}`)) {
    tips.push("식당이나 카페는 시간대에 따라 대기와 분위기가 크게 달라질 수 있어, 붐비는 시간 전후로 방문하면 더 여유롭습니다.");
  } else {
    tips.push("주변 경관과 동선을 함께 보는 장소라면, 도착 직후보다 한 바퀴 천천히 걸은 뒤 이야기를 듣는 편이 좋습니다.");
  }
...
function getRegionVisitTip(location = {}) {
  ...
  return makeCard({
    ...
    summary: "지명 유래나 설화 카드부터 골라 들으면 지역의 인상이 더 선명해집니다.",
    script: `${region.regionName}에 있다면 먼저 지명 유래, 설화, 역사 카드 중 끌리는 이야기 하나를 골라 들어보세요. 긴 오디오북처럼 모두 이어 들을 필요 없이, 지금 걷는 동선과 가장 잘 맞는 카드만 선택하면 됩니다.`,
    sourceType: "derived_context",
...
function estimateDurationSeconds(script = "") {
  ...
  return Math.max(15, Math.round(textLength / 5.2));
}
